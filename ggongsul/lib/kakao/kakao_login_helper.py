...
class KakaoLoginHelper:
    ...
    def __init__(
        ...
        self._client_id = client_id if client_id else settings.KAKAO_REST_API_KEY
        self._client_secret = (
            client_secret if client_secret else settings.KAKAO_API_CLIENT_SECRET
        )
        self._redirect_uri = (
            redirect_uri
            if redirect_uri
            else getattr(settings, "KAKAO_API_REDIRECT_URI", "")
        )
    ...
    def _request(
        self, url: str, method: str, data: dict = None, json: dict = None
    ) -> dict:
        if method == "get":
